...
import { SCRIPT_WRITER_SYSTEM, buildMultilingualGeneratePrompt } from '@/lib/ai-prompts'
...
export async function POST(request: Request) {
  try {
    ...
    const { topic, language, duration_seconds, tone, audience } = body
    ...
    const userPrompt = buildMultilingualGeneratePrompt({
      topic: topic.trim(),
      language,
    ...
    await getSupabaseAdmin()
      ...
      .insert({
        ...
        input_text: `${language}: ${topic.trim()}`,
        output_text: result.content,
      })

    return NextResponse.json({ script: result.content })
  } catch (error) {
    console.error('Generate multilingual script error:', error)
